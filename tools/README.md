...
downloaded build if it is signed by your offline private key. A compromised download server can
serve any bytes, but cannot forge a signature the app will accept — so it cannot push malware.
...
2. Sign it (reads `tools/update-private-key.txt`). Easiest: **double-click `tools/sign-release.cmd`**
   (it signs the default Release exe; or drag an exe onto it, or pass a path). Equivalent command:
   ```
...
3. (Optional but recommended) verify before uploading, using your public key:
   ...
   ```
4. Upload **both** files, side by side, to the update host (`https://rbxextr.vexthatprotogen.com/`):
   - `RBXAssetExtractor.exe`
   - `RBXAssetExtractor.exe.sig`
